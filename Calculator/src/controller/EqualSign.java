...
public class EqualSign {
	...
	public void calculateExpression(StringBuilder numberBuilder) { 
		if(expressionDTO.getOperator() == ' ') {  //첫번째 숫자 입력 후 연산자 입력없이 바로 '='입력시 -> 결과 : 첫번째 입력값
			expressionDTO.setFirstValue(numberBuilder.toString());
			...
			expressionDTO.setSecondValue(numberBuilder.toString()); //계산완료된 적이 없음 -> 연산자 입력 후 누적된 두번째 숫자값 저장          예외) 3+=입력시 첫번째값=두번째값 -> 3+3=6
			arithmeticOperation.calculateExpression(expressionDTO);   //현재까지 입력한 값 계산 후 DTO에 저장
		}

		expressionDTO.setExpression();                            //수학식 DTO에 저장
