    ...
    class SearchingBook
    {
        ...
        public string InputSearchWord(int left, int top, int messageTop)
        {
            ...
            while (Constants.INPUT_VALUE)
            {
                ...
                else if(string.IsNullOrEmpty(searchWord) || Regex.IsMatch(searchWord, Constants.BOOK_NAME_REGEX) == Constants.IS_NOT_MATCH)
                {
                    adminMenu.PrintInputBox(0, messageTop, Constants.BOOK_NAME_ERROR_MESSAGE);
                } 
                else break; //목록에 있는 책에 대한 검색어를 입력한 경우

                adminMenu.PrintInputBox(left, top, Constants.REMOVE_LINE);
            }
        ...
        public int SelectSearchType(Keyboard keyboard)
        {
            ...
            adminMenu.PrintSearchBox();
            adminMenu.PrintSearchingBook(bookDatabaseManager.MakeBookList((int)Constants.SearchMenu.ALL, ""));   //전체도서 출력
            ...
            if (searchType == Constants.ESCAPE) return (int)Constants.ESCAPE;  //검색유형 선택 도중 뒤로가기 -> 관리자 메뉴로 돌아감
        ...
        {
            int searchType = SelectSearchType(keyboard);    //검색유형 선택
            if (searchType == (int)Constants.ESCAPE) return;//검색유형 선택 중 esc -> 도서검색 종료

            string searchWord = InputSearchWord((int)Constants.SearchMenu.LEFT_VALUE_OF_INPUT, searchType, (int)Constants.SearchMenu.ERROR_MESSAGE);   //검색어 입력받기
            if (searchWord.Equals(Constants.ESC)) return;   //검색어 입력 중 esc -> 도서검색 종료 

            Console.Clear();
            adminMenu.PrintSearchingBook(bookDatabaseManager.MakeBookList(searchType, searchWord));   //검색결과로 나온 책목록 출력
